...
2. Then, use the "reviewer" agent (named `review`) to review `{outputs.implementation}` and classify findings under `## Critical (must fix)`, `## Warnings (should fix)`, and `## Suggestions (consider)`. Finding lines use `path:line - [issue] — [why] — [fix or defer]`. When Critical is empty, the reviewer must write `## Critical (must fix)\n- None.` Summary ends with `Verdict: Ship | Ship with fixes | Do not ship`.
3. Finally, use the "general" agent to address `{outputs.review}`. The general agent must:
   ...
   - Otherwise, resolve every item under `## Critical (must fix)` and explicitly report each fix (use the finding's fix direction when present).
   - Report any remaining `## Warnings (should fix)` items separately (fix or justify deferral).
   - Treat `Verdict: Do not ship` as requiring Critical resolution (or an explicit blocker) before claiming completion.
