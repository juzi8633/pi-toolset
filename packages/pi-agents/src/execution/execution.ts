...
import { GrokAcpClientError, runGrokAcpClient } from '../runtime/grok-acp/grok-acp-client.ts';
...
} from '../runtime/grok-acp/grok-acp-invocation.ts';
import {
  createGrokAcpParserState,
  finalizeGrokAcpPrompt,
  handleGrokAcpSessionUpdate,
} from '../runtime/grok-acp/grok-acp-parser.ts';
import { getGrokInvocation } from '../runtime/grok-acp/grok-command.ts';
import {
...
async function runSingleAgentGrokAcp(
  ...
  try {
    const { openGrokAcpConnection } = await import('../runtime/grok-acp/grok-acp-client.ts');
    const { buildGrokAcpSessionLoadParams } = await import('../runtime/grok-acp/grok-acp-invocation.ts');
    const { createGrokAcpTranscriptProjector } = await import('../runtime/grok-acp/grok-acp-transcript.ts');
...
async function runFreshTuiGrokAcp(
  ...
  const { GrokAcpInteractiveTransport } = await import('../runtime/grok-acp/grok-acp-interactive-transport.ts');
  const { acquireSessionLease, buildSessionLeaseKey } = await import('../run/session-lease.ts');
