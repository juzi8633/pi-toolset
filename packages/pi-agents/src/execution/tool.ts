...
import { startProfile, stopProfile } from '../shared/profiler.ts';
...
import { externalizeJsonPayload, externalizeTextPayload } from '../output/result-payload.ts';
...
import {
  ...
  startDurableRun,
  type StartedRun,
} from '../run/run-persistence.ts';
import type { RunAbortOrigin } from '../run/run-types.ts';
import type { RunStore } from '../run/run-store.ts';
import { chainFanoutUnitId, chainStepUnitId, pad } from '../run/run-coordinator.ts';
import type { RunCoordinator, UnitExecutionContext } from '../run/run-coordinator.ts';
...
export interface ExecuteAgentToolOptions {
  ...
  buildRestoredChainState?: (input: {
    mode: Mode;
    record: import('../run/run-types.ts').AgentRunRecordV1;
    units: Record<string, import('../run/run-types.ts').RunUnitRecord>;
    resolvedFanouts: Record<string, import('../run/run-types.ts').WorkflowFanoutState>;
...
function stampRunOnDetails(
  ...
  finalStatus: import('../run/run-types.ts').RunStatus
): void {
...
> {
  ...
  let record!: import('../run/run-types.ts').AgentRunRecordV1;
  ...
  let units!: Record<string, import('../run/run-types.ts').RunUnitRecord>;
    ...
    ...(runStore
      ? {
          resolveArtifact: async (
            ref: import('../run/run-types.ts').RunArtifactRefV1
          ): Promise<unknown> => {
