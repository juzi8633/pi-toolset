...
import { type AgentConfig, type AgentOverride, discoverAgents } from '../config/agents.ts';
import { openAgentConfigUi } from '../config/agent-config-ui.ts';
import type { BackgroundManager } from './background.ts';
import type { SessionAgentConfigStore } from '../config/session-agent-config.ts';
...
export interface RegisterAgentCommandOptions {
  ...
  runStore?: import('../run/run-store.ts').RunStore;
  runCoordinator?: import('../run/run-coordinator.ts').RunCoordinator;
...
async function runAgentFallbackCommand(
  ...
  if (lower === 'runs' && options.runStore) {
    ...
    const lines = runs
      .filter((r): r is import('../run/run-types.ts').LoadedRun => 'record' in r)
      .map((r) => {
        const rec = r.record;
