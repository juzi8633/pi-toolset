...
import { isRunArtifactRef } from '../run/artifact-store.ts';
import {
  formatParentArtifactDescriptor,
  shouldSpillPayload,
  textPayloadBytes,
} from '../output/result-payload.ts';
import type { RunArtifactRefV1 } from '../run/run-types.ts';
...
export interface InteractiveContinuationDetails {
  ...
  outputRef?: import('../run/run-types.ts').RunArtifactRefV1;
