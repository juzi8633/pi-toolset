...
import type { AgentScope, AgentSource } from '../config/agents.ts';
import type { ResumeCapability, RunArtifactRefV1 } from '../run/run-types.ts';
...
export interface SubagentDetails {
  ...
  run?: {
    runId: string;
    status: import('../run/run-types.ts').RunStatus;
    resumable: boolean;
