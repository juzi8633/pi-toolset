...
- `runSingleAgent` already keeps running after its caller stops awaiting: its child stdout `'data'` listeners keep the event loop engaged and accumulate into `currentResult` until the child `close`. We only need to *not await* it and *not* reuse the per-call `onUpdate`/`signal`.
