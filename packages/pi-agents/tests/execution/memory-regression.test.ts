...
import { discoverAgents, type AgentConfig } from '../../src/config/agents.ts';
import type { SingleResult, SubagentDetails } from '../../src/shared/types.ts';
import { emptyUsage } from '../../src/shared/types.ts';
import { createLatestValueCoalescer } from '../../src/shared/update-coalescer.ts';
...
describe('memory regressions', () => {
  ...
  it('externalizes 12 MiB final text so terminal SingleResult JSON stays under 1 MiB', async () => {
    ...
    const { externalizeTerminalResult } = await import('../../src/output/result-payload.ts');
