...
import { chainFanoutStepId, chainFanoutUnitId } from '../../src/run/run-coordinator.ts';
import type { ChainOutputEntry, SingleResult, SubagentDetails } from '../../src/shared/types.ts';
import type { RunArtifactRefV1 } from '../../src/run/run-types.ts';
...
describe('runChainWorkflow', () => {
  ...
  it('restored fanout uses collect textRef over last child finalOutputRef for {previous}', async () => {
    ...
    const units: Record<string, import('../../src/run/run-types.ts').RunUnitRecord> = {
...
describe('runChainWorkflow chain-output externalization authority', () => {
  ...
  it('restored named structuredRef followed by JSON Pointer/fanout resolves at runtime', async () => {
    ...
    const restored: import('../../src/execution/chain.ts').RestoredChainState = {
      results: [
        {
  ...
  it('sequential production-like callback runs once; ignored stubs still get one fallback', async () => {
    ...
    const resProd = await runChainWorkflow({
      ...
      runStep: async (req) => {
        ...
        const { snapshotSingleResult } = await import('../../src/output/result-snapshot.ts');
