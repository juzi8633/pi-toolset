...
import type { AgentConfig, Runtime } from '../../src/config/agents.ts';
import {
  aggregateCapability,
  agentFingerprint,
  assertUniqueUnitIds,
...
describe('persistSessionFile', () => {
  ...
  it('finishUnit set-or-delete: ctx stale identity cleared when unit has none', async () => {
    ...
    const ctx: import('../../src/run/run-coordinator.ts').UnitExecutionContext = {
      runId: 'run-stale-clear',
      unitId: 'single',
      agent: 'noop',
      runtime: undefined,
...
describe('startUnit disk-first strict failures', () => {
  async function createQueuedRun(
    root: string,
    storeOptions?: Omit<import('../../src/run/run-store.ts').CreateRunStoreOptions, 'rootDir'>
  ): Promise<{
