...
import {
  ...
  type RelayPiApi,
} from '../../src/interactive/interactive-relay.ts';
import {
...
import type { InteractiveAgentLinkV1 } from '../../src/run/run-types.ts';
import type { ExtensionContext } from '@earendil-works/pi-coding-agent';
import type { Theme } from '@earendil-works/pi-coding-agent';
import type { PiRpcTransport } from '../../src/runtime/pi-rpc/pi-rpc-transport.ts';
import { INTERACTIVE_IDLE_TRANSCRIPT_MAX_BYTES } from '../../src/shared/constants.ts';
  ...
  function makeGatedRunStore() {
    ...
    const store = {
      async writeTextArtifact(
        ...
        _payload: string,
        _text: string
      ): Promise<import('../../src/run/run-types.ts').RunArtifactRefV1> {
        await gate;
        return {
          kind: 'run-artifact',
          version: 1,
    ...
    return {
      store: store as unknown as import('../../src/run/run-store.ts').RunStore,
      release: () => gateResolve?.(),
    };
  }
